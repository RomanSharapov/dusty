...
class Processor(DependentModuleModel, ProcessorModel):
    ...
    def execute(self):
        ...
        for processor in ["false_positive"]:
            log.info("Adding %s processor", processor)
            self.context.performers["processing"].schedule_processor(processor, self.config)
    ...
    def fill_config(data_obj):
        """ Make sample config """
        data_obj.insert(
            len(data_obj), "config", "/path/to/test.config",
            comment="File with config"
        )
